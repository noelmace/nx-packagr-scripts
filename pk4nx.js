...
program.version(require(join(__dirname, 'package.json')).version)
...
program.command('build').description('build libraries').action(async () => {
  try {
    const rslt = await build(cwd, program)
    const msg = Array.isArray(rslt) ? 'All libraries from libs/ were successfully built.' +
...
program.command('publish').description('publish prebuilt libraries').action(async () => {
  try {
    const rslt = await publish(cwd, program)
    console.log(`${Array.isArray(rslt) ? 'All previously buildt libraries were' : rslt + ' has been'} successfully published.`)
    process.exit(0)
  } catch (e) {
    console.error(program.debug ? e : (e.message || e))
    process.exit(1)
